...
`gpu-server/setup_positions.py` 1회 실행 → 로봇을 손으로 움직여 **테이블 높이 · 초기 위치 · 스캔 위치 · 놓는 위치**를 잡으면 `config.ini [positions]` 가 자동으로 채워짐. 이후 `pick/pick_place.py` 실행 시 무조건 **① 초기(initial) 위치 → ② 스캔 위치** 순으로 자동 이동. (둘 다 config 좌표, 코드 수정 불필요)
